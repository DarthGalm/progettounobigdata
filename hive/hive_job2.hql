...
FROM(
SELECT grouped_by_ticker.sector, grouped_by_ticker.year, (sum(grouped_by_ticker.sumVolume) / sum(grouped_by_ticker.avgCounter)) as avgVolume, avg(grouped_by_ticker.percent_variation) as avgVariation, (sum(grouped_by_ticker.sumClose) / sum(grouped_by_ticker.avgCounter)) as avgClose 
FROM(
